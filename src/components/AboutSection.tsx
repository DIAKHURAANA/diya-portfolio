...
export const AboutSection: React.FC<AboutSectionProps> = ({ onContactClick }) => {
  ...
  return (
    ...
    >
      {/* Background Cinematic Light Movement - Paused on mobile */}
      {!isMobile && isInView && (
        <motion.div
          animate={{
            x: [-50, 50, -50],
      ...
      <div className="max-w-7xl mx-auto w-full relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-center">
        {/* LEFT COLUMN: Line-by-Line Reveal */}
        <motion.div
          initial={{ opacity: 0, x: isMobile ? 0 : -50, y: isMobile ? 20 : 0, filter: isMobile ? undefined : 'blur(10px)' }}
        ...
        >
          ...
          <div className="overflow-hidden">
            <motion.h2
              initial={{ clipPath: isMobile ? undefined : 'polygon(0 0, 0 0, 0 100%, 0 100%)', y: 30, opacity: 0 }}
          ...
          <div className="space-y-4 font-syne text-base sm:text-lg font-light text-[#EAE0D5]/90 leading-relaxed max-w-2xl">
            <motion.p
              initial={{ opacity: 0, y: 20, filter: isMobile ? undefined : 'blur(6px)' }}
          ...
          >
            {bulletPoints.map((point, index) => (
              <motion.div
                ...
                viewport={{ once: true }}
                transition={{ duration: 0.5, delay: Math.min(index * 0.08 + 0.3, 0.5), ease: cubicEase }}
                className="p-4 rounded-xl bg-black/40 border border-white/10 md:backdrop-blur-md flex items-center gap-3 hover:border-[#C84B31]/60 transition-colors"
              >
        ...
        <motion.div
          initial={{ opacity: 0, x: isMobile ? 0 : 50, y: isMobile ? 20 : 0, filter: isMobile ? undefined : 'blur(12px)' }}
          whileInView={{ opacity: 1, x: 0, y: 0, filter: 'blur(0px)' }}
          viewport={{ once: true, margin: '-80px' }}
          transition={{ duration: 0.9, ease: cubicEase }}
          className="lg:col-span-5 relative"
        >
          {/* Floating Loop Motion Container - Reduced motion on mobile */}
          <motion.div
            animate={isMobile ? { y: 0 } : { y: [0, -8, 0, 8, 0] }}
            transition={isMobile ? { duration: 0 } : { duration: 6, repeat: Infinity, ease: 'easeInOut' }}
            onMouseMove={handleMouseMove}
            onMouseLeave={handleMouseLeave}
            style={{
              transform: isMobile ? undefined : `perspective(1000px) rotateX(${tilt.x}deg) rotateY(${tilt.y}deg)`,
              transition: 'transform 0.15s ease-out',
            }}
            className="relative rounded-3xl overflow-hidden border border-white/20 bg-black/50 md:backdrop-blur-xl shadow-2xl group cursor-pointer"
          >
            ...
            <div className="absolute bottom-6 left-6 right-6 p-6 rounded-2xl bg-black/70 md:backdrop-blur-md border border-white/15 flex items-center justify-between">
              <div>
